...
class NEDWriter:
    ...
    def write(self) -> bool:
        self.write_network_ned(f=open(self.filename,'w'), nodeList=self.nodeList,
...
class INIWriter:
    def __init__(self, filename: str, nodeList: list, channelList: list):
        ...
        self.channelList = channelList
    def write(self) -> bool:
        self.write_omnetpp_ini(f=open(self.filename,'w'), nodeList=self.nodeList,
                               channelList=self.channelList)
        # return True
    # 写入 omnetpp.ini 的具体内容
    def write_omnetpp_ini(self, f, nodeList=None, channelList=None):
        f.write('network = inet.examples.computing_power_network.simpletest.Network\n\n')
        ...
        # 算力网关列表
        computing_gateways = [node for node in nodeList if node.nodetype == 'ComputingGateway']
        # 调度决策网关列表
        compute_schedule_nodes = [node for node in nodeList if node.nodetype == 'ComputeScheduleNode']
        # 路由器列表
    ...
    for node in nodeList:
        if node.nodetype in ["Router", "UserGateway", "ComputingGateway"]:
            ...
            for i, channel in enumerate(node.channelList):
                ...
                if channel.another_point_of_channel(node).nodetype in ["UserNode", "ComputeNode"]:
