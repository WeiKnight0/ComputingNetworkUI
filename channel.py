...
class Channel(QGraphicsLineItem, QObject):
    ...
    def update_position(self):
        # 根据节点位置更新连线
        start_pos = self.start_item.scenePos() + QPointF(
            self.start_item.icon.width() / 2 * self.start_item.scale_factor,
            self.start_item.icon.height() / 2 * self.start_item.scale_factor,
        )
        end_pos = self.end_item.scenePos() + QPointF(
            self.end_item.icon.width() / 2 * self.end_item.scale_factor,
    ...
    def __getstate__(self):
        return {
            ...
            "start_interface_index": (
                self.start_interface_index if self.start_interface_index != 0 else None
            ),
            "end_interface_index": (
                self.end_interface_index if self.end_interface_index != 0 else None
            ),
        }
    ...
    def delete_channel(self):
        ...
        print(
            f"start_item为{self.start_item.name},counter为{self.start_item.interface_counter}"
        )
        print(
            f"end_item为{self.end_item.name}, counter为{self.end_item.interface_counter}"
        )

        for interface_id_and_object in self.start_item.interface_id_to_object[::-1]:
            if (
                interface_id_and_object[1] == type(self.end_item)
                and interface_id_and_object[2] == self.end_item.index
            ):
            ...
            if interface_id_and_object[0] > self.start_interface_index:
                ...
                print(
                    f"对面对象的type为{interface_id_and_object[1]},index为{interface_id_and_object[2]}"
                )
        ...
        for interface_id_and_object in self.end_item.interface_id_to_object[::-1]:
            if (
                interface_id_and_object[1] == type(self.start_item)
                and interface_id_and_object[2] == self.start_item.index
            ):
        ...
        for channel in self.start_item.channelList:
            if (
                channel.start_item == self.start_item
                and channel.start_interface_index > self.start_interface_index
            ):
                channel.start_interface_index -= 1
            elif (
                channel.end_item == self.start_item
                and channel.end_interface_index > self.start_interface_index
            ):
                channel.end_interface_index -= 1

        for channel in self.end_item.channelList:
            if (
                channel.start_item == self.end_item
                and channel.start_interface_index > self.end_interface_index
            ):
                channel.start_interface_index -= 1
            elif (
                channel.end_item == self.end_item
                and channel.end_interface_index > self.end_interface_index
            ):
                channel.end_interface_index -= 1
    ...
    def update_channel(self, new_bandwidth, new_banddelay):
        if new_bandwidth >= 0 and new_banddelay >= 0:
            ...
            print(
                f"通道带宽更新为: {self.bandwidth} Mbps，时延更新为: {self.banddelay} ms"
            )
        else:
            print("更新失败")
