...
class SetUserRouterWidget(QWidget):
    # 定义一个信号，用于传递更新后的带宽值
    node_updated = Signal()

    def __init__(self, user_router, parent=None):
        ...
        # 加载UI文件
        loader = QUiLoader()
        self.ui = loader.load('set_userRouter.ui')  # 加载UI界面
        ...
        # 设置当前的值
        self.ip_line_edit_1.setText(self.user_router.ip)
        self.mask_line_edit_1.setText(self.user_router.mask)

        self.ip_line_edit_2.setText(self.user_router.ip_2)
        self.mask_line_edit_2.setText(self.user_router.mask_2)
    ...
    def accept_update(self):
        ...
        self.user_router.ip = self.ip_line_edit_1.text()
        self.user_router.mask = self.mask_line_edit_1.text()

        self.user_router.ip_2 = self.ip_line_edit_2.text()
        self.user_router.mask_2 = self.mask_line_edit_2.text()

        print(f"更新成功: IP={self.user_router.ip}, Mask={self.user_router.mask}")
